...
# ── 配置 ──────────────────────────────────────────────────────────────
MODEL_PATH = "/root/autodl-tmp/basic-model/Qwen/Qwen2-1.5B"
N_PROMPTS = 50
MAX_NEW_TOKENS = 100
BATCH_SIZE = 8
...
def bench_transformers(prompts: list[str]) -> dict:
    print("\n" + "=" * 70)
    print("  加载 transformers Qwen2-1.5B-Instruct")
    print("=" * 70)
...
def bench_vllm(prompts: list[str]) -> dict:
    print("\n" + "=" * 70)
    print("  加载 vLLM Qwen2-1.5B-Instruct")
    print("=" * 70)
...
def plot_results(r: dict, out_path: str):
    ...
    plt.suptitle("vLLM vs Transformers: Throughput Benchmark (Qwen2-1.5B, RTX 4090D 24GB)",
