...
```
依赖：`openai`、`faiss-cpu`、`numpy`、`httpx`、`mcp>=1.0.0`、`fastapi`、`uvicorn`（Web 前端）。无需 Node.js（本版不使用 Filesystem MCP）。
...
> 完整操作说明（多人 / 短期记忆 / Web 会话生命周期）见：[mode_function_call/操作文档.md](mode_function_call/操作文档.md)

**命令行：**
```bash
python mode_function_call/run_function_call.py                  # 交互（支持 @用户 切换）
python mode_function_call/run_function_call.py -q "宁德今天天气怎么样？"
python mode_function_call/run_function_call.py --demo            # 单用户多轮记忆演示
python mode_function_call/run_function_call.py --multi --rounds 2  # 多人并发演示
```

**Web 前端（一页一会话，关页销毁记忆）：**
```bash
python -m mode_function_call.web_app --port 8765
# 浏览器打开 http://127.0.0.1:8765/
```

**内部流程**：
1. 手写 `TOOLS_SCHEMA`（天气工具 JSON Schema）
2. `chat.completions.create(tools=TOOLS_SCHEMA, tool_choice="auto")`
3. 若返回 `tool_calls`：查 `TOOL_DISPATCH` 调 `get_weather` → `role=tool` 回填
4. 再次 `create`，模型生成最终回答；结果写入该用户/会话的短期记忆

**预期输出**：先打印 `→ [tool] get_weather({...})` 与结果预览，再打印天气分析回答。
...
result = run_query(client, model, "宁德今天天气怎么样？", user_id="u1", memory=memory)
print(result["answer"], result["tool_calls"], result["memory"])
```
...
**Q9：Web 页面 401 / Authentication Fails**
`DEEPSEEK_API_KEY` 无效。换新 Key 后重启 `web_app`。详见 [操作文档 §6](mode_function_call/操作文档.md)。

**Q10：关闭网页后会话还在？**
一般 `sendBeacon` 立即销毁；异常断开最多约 45s 心跳超时清理。
